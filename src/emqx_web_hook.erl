...
send_http_request(ClientID, Params, Path, Headers) ->
    Body = emqx_json:encode(Params),
    ?LOG_GLD("Send to: ~0p, params: ~0s", [Path, Body]),
    case ehttpc:request(ehttpc_pool:pick_worker(?WEB_HOOK_APP, ClientID), post, {Path, Headers, Body}) of
